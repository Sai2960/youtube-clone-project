...
export const getVideoUrl = (
  ...
  console.log(
    "🎬 Processing video URL for:",
    video._id,
    "Quality:",
    quality || "default"
  );

  // ✅ Quality transformation mapping (moved outside to support dynamic quality)
  const getQualityTransformation = (q?: string): string => {
    if (!q || q === "auto") {
      return "f_mp4,vc_h264,ac_aac,af_44100,q_auto:good,br_auto";
    }

    const qualityMap: Record<string, string> = {
      "1080p":
        "w_1920,h_1080,c_limit,f_mp4,vc_h264,ac_aac,af_44100,q_auto:good,br_5000k",
      "720p":
        "w_1280,h_720,c_limit,f_mp4,vc_h264,ac_aac,af_44100,q_auto:good,br_2500k",
      "480p":
        "w_854,h_480,c_limit,f_mp4,vc_h264,ac_aac,af_44100,q_auto:good,br_1000k",
      "360p":
        "w_640,h_360,c_limit,f_mp4,vc_h264,ac_aac,af_44100,q_auto:good,br_500k",
    };

    return qualityMap[q] || qualityMap["auto"];
  };
  ...
  if (
    ...
    const url = `${CLOUDINARY_BASE}/${transformation}/${video.videofilename}.mp4`;
    console.log(
      "✅ Built URL from videofilename with quality:",
      quality || "auto"
    );
    return url;
  }
  ...
  for (const field of cloudinaryFields) {
    ...
    if (
      ...
      // ✅ Remove version numbers and existing transformations
      let cleanUrl = urlStr.replace(/\/v\d+\//g, "/");

      // Extract public_id
      const publicIdMatch = cleanUrl.match(
        /youtube-clone\/videos\/file_\d+_[a-z0-9]+/i
      );
      if (publicIdMatch) {
        const publicId = publicIdMatch[0];
        const rebuiltUrl = `${CLOUDINARY_BASE}/${transformation}/${publicId}.mp4`;
        console.log("✅ Rebuilt video URL with quality:", quality || "auto");
        return rebuiltUrl;
      }

      // ✅ If we can't extract public_id, strip existing transformation and add new one
      if (cleanUrl.includes("/upload/")) {
        cleanUrl = cleanUrl.replace(/\/upload\/[^/]+\//, "/upload/");
        const secureUrl = cleanUrl
          .replace(/^http:\/\//, "https://")
          .replace("/upload/", `/upload/${transformation}/`);
        console.log("✅ Using modified Cloudinary URL with quality");
        return secureUrl;
      }
    }
  }
  ...
  for (const field of cloudinaryFields) {
    ...
    if (publicIdMatch) {
      const publicId = publicIdMatch[0];
      const reconstructedUrl = `${CLOUDINARY_BASE}/${transformation}/${publicId}.mp4`;
      console.log(
        "🔧 Reconstructed URL from public_id with quality:",
        quality || "auto"
      );
  ...
  // ✅ PRIORITY 4: Handle non-Cloudinary URLs (legacy support - no quality control)
  const rawUrl =
    video.filepath ||
    video.videoLink ||
    video.videofile ||
  ...
  if (rawUrl) {
    ...
    if (urlStr.startsWith("https://") || urlStr.startsWith("http://")) {
      ...
      console.log("✅ Using full non-Cloudinary URL (no quality control)");
      return secureUrl;
    }
...
export const getThumbnailUrl = (video: any): string | null => {
  ...
  for (const source of videoSources) {
    try {
      ...
      const publicIdMatch = cleanUrlStr.match(
        /youtube-clone\/videos\/file_\d+_[a-z0-9]+/i
      );
