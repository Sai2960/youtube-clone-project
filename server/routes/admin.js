...
router.post(
  "/test/expire-subscription/:subscriptionId",
  verifyToken,
  isAdmin,
  async (req, res) => {
    try {
...
router.get("/fix-history-indexes", verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    // Video duplicates
    const videoDuplicates = await historyCollection
      .aggregate([
        {
          $match: {
            contentType: "video",
    ...
    // Short duplicates
    const shortDuplicates = await historyCollection
      .aggregate([
        {
          $match: {
            contentType: "short",
    ...
    results.push(
      `Total: ${stats.total} | Videos: ${stats.videos} | Shorts: ${stats.shorts}`
    );
    results.push("\n✅ Index fix complete!");
...
router.get("/fix-content-types", verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    if (finalStats.stillMissing > 0) {
      ...
      const problematic = await historyCollection
        .find({
          $or: [{ contentType: { $exists: false } }, { contentType: null }],
        })
        .limit(5)
...
router.put("/approve-user/:userId", verifyToken, isAdmin, approveUser);
router.put("/reject-user/:userId", verifyToken, isAdmin, rejectUser);
router.get("/all-users", verifyToken, isAdmin, getAllUsersWithStatus);
