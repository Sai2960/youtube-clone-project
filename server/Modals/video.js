...
const videoSchema = new mongoose.Schema(
  {
    ...
    qualities: {
      mobile_low: String, // 360p for slow connections
      mobile: String, // 480p for mobile 4G
      sd: String, // 720p for desktop
      hd: String, // 1080p for high-speed
      original: String, // Original quality
    },

    // ✅ NEW: Multiple thumbnail sizes
    thumbnails: {
...
videoSchema.pre("save", function (next) {
  ...
  if (!this.description && this.videodescription)
    this.description = this.videodescription;
  if (!this.videodescription && this.description)
  ...
  if (
    ...
    (!this.shares || this.shares.total === undefined)
  ) {
    if (!this.shares) {
      this.shares = {
        total: 0,
...
// =================== EXPORT ===================
const videofiles =
  mongoose.models.videofiles || mongoose.model("videofiles", videoSchema);
