      ...
      validate: {
        validator: function (v) {
          for (let [key, value] of v) {
            if (!["all", "personalized", "none"].includes(value)) {
              return false;
            }
          }
          return true;
        },
        message: "Notification preference must be all, personalized, or none",
      },
    },
...
userSchema.pre("save", function (next) {
  ...
  if (!this.channelname) {
    this.channelname =
      this.name || (this.email ? this.email.split("@")[0] : "Channel");
    console.log("✅ Auto-generated channelname:", this.channelname);
  }

  if (!Array.isArray(this.subscribedChannels)) {
    console.warn(
      "⚠️ Fixing subscribedChannels - was:",
      typeof this.subscribedChannels
    );
  ...
  if (
    typeof this.totalVideoUploads !== "number" ||
...
userSchema.methods.subscribeTo = async function (channelId) {
  ...
  await mongoose
    .model("User")
    .findByIdAndUpdate(channelId, { $inc: { subscribers: 1 } });
...
userSchema.methods.isActiveUser = function () {
  ...
  const daysSinceLogin =
    (Date.now() - this.lastLoginTime) / (1000 * 60 * 60 * 24);
